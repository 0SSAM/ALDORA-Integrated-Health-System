...
describe("showcase scope reconciliation", () => {
  it("repairs scope only for an active user in an isolated showcase organization", async () => {
    ...
    expect(source).toContain("export async function reconcileManagedShowcaseAccount");
  ...
  it("reconciles only after password verification and before the scope-backed session is created", async () => {
    ...
    const verifiedPassword = source.indexOf("verifyInternalPassword(input.password, credential.passwordHash)");
    const reconciliation = source.indexOf("await reconcileShowcaseScope(credential.userId)");
    const scopeLookup = source.indexOf("const scope = await getInternalScopeForUser(credential.userId)");
  ...
  it("runs managed reconciliation once during server startup without blocking the service", async () => {
    ...
    expect(source).toContain("the isolated showcase account remains unavailable");
  });
});
