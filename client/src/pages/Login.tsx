...
import { ShieldCheck, ShoppingCart, ArrowLeft, Loader2, UserRound } from "lucide-react";
        ...
        <div className="p-8 sm:p-12">
          ...
          <div className="max-w-md">
            ...
            {user ? <div className="mt-8 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900"><p className="font-semibold">أنت مسجل الدخول بالفعل</p><p className="mt-1 text-sm leading-6">الحساب: {user.name || "مستخدم مصادق"} · الدور: {user.role}</p><p className="mt-2 text-xs leading-5 text-emerald-800">هذه جلسة إدارة عبر OAuth. لتسجيل دخول موظف بيوزر وكلمة مرور، سجّل الخروج أولاً ثم سيظهر نموذج الموظفين الداخليين.</p><div className="mt-4 flex flex-wrap gap-2"><Button type="button" onClick={() => setLocation("/")} className="bg-[#0d1b2a] hover:bg-[#16324a]">فتح مساحة العمل</Button><Button type="button" variant="outline" onClick={() => void logout()} className="border-emerald-300 bg-white">تسجيل الخروج ثم دخول موظف</Button><Button type="button" variant="outline" onClick={async () => { await logout(); await startDemo(); }} className="border-violet-300 bg-violet-50 text-violet-900 hover:bg-violet-100">تجربة Demo التفاعلية</Button></div></div> : <>
              ...
              <Button type="button" variant="outline" onClick={() => void startDemo()} className="mt-3 h-12 w-full border-cyan-200 bg-cyan-50/60 text-cyan-900 hover:bg-cyan-100"><ShoppingCart className="ml-2 h-4 w-4" />ابدأ تجربة Demo التفاعلية</Button>
              <p className="mt-2 text-center text-xs text-cyan-700">سيفتح لك لوحة الوحدات التفاعلية ببيانات اصطناعية قابلة لإعادة الضبط.</p>
              <p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-800">وضع العرض آمن ومحلي: لا تُحفظ بيانات إنتاجية ولا تُرسل عمليات حقيقية. يمكنك تجربة نقطة البيع والمخزون والوصفات والكتالوج وخدمة العملاء والتقارير.</p>
            </>}
          </div>
        </div>
      </section>
