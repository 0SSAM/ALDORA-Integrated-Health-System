...
import { InsuranceWorkspace, OperationsHubWorkspace, OrganizationWorkspace, PromotionsWorkspace, ReportsWorkspace } from "@/components/IntegratedOperationsWorkspaces";
...
import { TaxInvoiceWorkspace } from "@/components/TaxInvoiceWorkspace";
...
const modules = [
  ...
  { id: "operationsHub", label: "مركز التشغيل والباقات", searchText: "operations hub pharmacy hospital distributor laboratory insurer rehabilitation services bundles", icon: HeartPulse },
];

const organizationModules: Record<string, string[]> = { government: ["overview", "compliance", "finance", "people", "insurance", "egyptHealthcare", "icd10", "operationsHub"], pharmacy: ["overview", "pos", "inventory", "supplyChain", "prescriptions", "insurance", "promotions", "compliance", "customerCare", "callCentre", "catalog", "icd10", "operationsHub"], pharmacy_chain: ["overview", "pos", "inventory", "supplyChain", "prescriptions", "insurance", "promotions", "compliance", "finance", "people", "customerCare", "callCentre", "catalog", "icd10", "operationsHub"], distributor: ["overview", "inventory", "supplyChain", "promotions", "compliance", "finance", "people", "catalog", "operationsHub"], insurer: ["overview", "insurance", "compliance", "finance", "people", "customerCare", "icd10", "operationsHub"], rehabilitation: ["overview", "prescriptions", "customerCare", "finance", "compliance", "people", "icd10", "operationsHub"], hospital: ["overview", "inventory", "prescriptions", "insurance", "egyptHealthcare", "compliance", "finance", "people", "customerCare", "icd10", "operationsHub"], laboratory: ["overview", "prescriptions", "compliance", "finance", "people", "customerCare", "icd10", "operationsHub"], radiology: ["overview", "prescriptions", "compliance", "finance", "people", "customerCare", "icd10", "operationsHub"] };

const showcaseModuleIds = new Set([
  "overview", "pos", "inventory", "supplyChain", "prescriptions", "insurance",
  "egyptHealthcare", "compliance", "compounding", "finance", "promotions", "people",
...
const showcaseTourSteps = [
  ...
  { module: "pos", title: "سيناريو بيع تدريبي: افتح نقطة البيع", body: "هذه بداية محاكاة بيع. افتح بطاقة سيناريو الفاتورة التجريبية وتعرّف إلى عناصر الطلب، مع بقاء أزرار الحفظ والتنفيذ مقفلة في بيئة العرض." },
  { module: "pos", title: "مسح باركود تجريبي", body: "اضغط «محاكاة مسح الباركود» لقراءة الرمز DEMO-622300-VC1000 محلياً. ستظهر بيانات الصنف والتشغيلة المقترحة فقط؛ لا تُستخدم الكاميرا، ولا يُنشأ قيد أو خصم مخزون." },
  { module: "pos", title: "سيناريو بيع تدريبي: اختر الصنف", body: "اختر الصنف التجريبي «فيتامين C 1000mg» من سيناريو العرض. في النظام الفعلي تُراجع الصلاحية والتشغيلة قبل الإضافة؛ هنا لا تُنشأ حركة مخزون." },
  { module: "pos", title: "سيناريو بيع تدريبي: راجع الكمية والعميل", body: "راجع الكمية التجريبية وبيانات العميل المختصرة وإشعار الحساسية أو الوصفة عند الحاجة. هذه البيانات اصطناعية ولا تُخزن أو تُرسل لأي جهة." },
  { module: "pos", title: "سيناريو بيع تدريبي: اختر طريقة الدفع", body: "اضغط «نقداً» أو «بطاقة» في محاكاة التسوية. سيعرض النظام الباقي النقدي أو نتيجة اعتماد بطاقة تجريبية فقط؛ لا يتم خصم أي مبلغ ولا يُنشأ قيد مالي أو مطالبة." },
  { module: "pos", title: "سيناريو بيع تدريبي: اعرض الضريبة والدفع", body: "راجع الإجمالي وضريبة القيمة المضافة ونتيجة التسوية التجريبية ضمن الفاتورة النموذجية. لا تُسجل دفعة ولا تُنشأ مطالبة ولا يتغير أي رصيد." },
  { module: "pos", title: "سيناريو بيع تدريبي: عاين الفاتورة", body: "هذه هي خطوة المراجعة النهائية: تتحقق من التفاصيل ثم تطبع أو تعتمد الفاتورة في التشغيل الحقيقي بعد الصلاحية. في العرض تكون المعاينة فقط ولا يوجد إصدار أو تصدير." },
  { module: "pos", title: "إيصال تجريبي: عاين واطبع محلياً", body: "افتح معاينة الإيصال ثم اختر الطباعة المحلية لرؤية شكل الإيصال النهائي. يحمل الإيصال علامة «عرض تدريبي» ولا يُحفظ أو يُصدر أو يُرسل إلى طابعة أو بوابة دفع فعلية." },
  { module: "pos", title: "محاكاة مرتجع: اختر الفاتورة المرجعية", body: "اختر الفاتورة التجريبية DEMO-INV-2026-1042، ثم راجع سبب المرتجع وكميات الأصناف. في التشغيل الفعلي تتحقق السياسة والضريبة وصلاحية المرتجع؛ هنا لا تُنشأ حركة عكسية." },
  { module: "pos", title: "محاكاة مرتجع: عاين إشعار المرتجع", body: "عاين إشعار الإرجاع التجريبي وقيمة الضريبة المستردة قبل الاعتماد. لا يصدر هذا الإشعار، ولا يعيد للمخزون، ولا يغير رصيد العميل أو المؤسسة." },
...
const coreShortcuts: ReadonlyArray<{ key: string; label: string; description: string; module: string; roles: readonly string[] }> = [
  ...
  { key: "F10", label: "مركز التشغيل", description: "فتح الباقات القطاعية وحالة الجاهزية والتنبيهات", module: "operationsHub", roles: ["admin", "manager", "pharmacist", "cashier", "user"] },
...
export default function Home() {
  ...
  }, [role, activeOrganizationType, isShowcaseSession]);
  ...
  const retreatShowcaseTour = () => {
    const previous = showcaseTourSteps[showcaseTourStep - 1];
    if (!previous) return;
    setActive(previous.module);
    setMobileOpen(false);
    setShowcaseTourStep(current => current - 1);
  };
    ...
    <div dir={localization.direction} data-country={localization.countryCode} className="aldo-app-shell min-h-screen text-slate-900">
      <aside className={cn("fixed inset-y-0 right-0 z-40 flex w-[286px] flex-col border-l border-slate-200 bg-[#0d1b2a] text-white transition-transform duration-200 lg:translate-x-0", mobileOpen ? "translate-x-0" : "translate-x-full")}>
        <div className="flex h-20 items-center justify-between border-b border-white/10 px-6">
          <div className="flex items-center gap-3"><div className="aldo-brand-mark grid h-10 w-10 place-items-center rounded-2xl" aria-label="ألدورا"><svg viewBox="0 0 48 48" role="img" aria-hidden="true" className="h-7 w-7"><path d="M24 4 40 10v12c0 10.5-6.7 18.2-16 22-9.3-3.8-16-11.5-16-22V10L24 4Z" fill="#0d1b2a" opacity=".92"/><path d="M24 12c-5.5 3.8-8.5 8.1-8.5 12.9 0 5.6 3.8 9.5 8.5 11.1 4.7-1.6 8.5-5.5 8.5-11.1C32.5 20.1 29.5 15.8 24 12Z" fill="#9ff2e4"/><path d="M24 18c-2.4 2.5-3.6 4.8-3.6 7.1 0 2.4 1.4 4.2 3.6 5.2 2.2-1 3.6-2.8 3.6-5.2 0-2.3-1.2-4.6-3.6-7.1Z" fill="#19c5d1"/></svg></div><div><p className="font-bold tracking-tight">ألدورا | منظومة الرعاية الصحية المتكاملة</p><p className="text-[11px] text-cyan-200/70">ALDORA | Integrated Health System</p></div></div>
        ...
        <div className="px-4 py-5"><p className="mb-3 px-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">مساحة العمل</p><p className="mb-3 px-3 text-xs text-cyan-200/70">{isShowcaseSession ? "حساب عرض · استكشاف آمن" : `الدور: ${user.role}`}</p>{isShowcaseSession && <><div className="mb-3 rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-3 py-2 text-xs leading-5 text-cyan-100">يمكنك فتح الوحدات أدناه لاستكشاف الواجهة. جميع محاولات الحفظ والتصدير والتكاملات الخارجية موقوفة تلقائياً.</div><Button type="button" variant="outline" className="mb-4 w-full border-cyan-300/30 bg-cyan-300/10 text-cyan-50 hover:bg-cyan-300/20 hover:text-white" onClick={() => { restartShowcaseTour(); setMobileOpen(false); }}>إعادة بدء الجولة</Button></>}<nav className="space-y-1">{filteredModules.map(item => { const Icon = item.icon; const selected = active === item.id; return <button key={item.id} onClick={() => { setActive(item.id); setMobileOpen(false); }} className={cn("flex w-full items-center gap-3 rounded-xl px-3 py-3 text-right text-sm transition", selected ? "bg-cyan-400 font-semibold text-[#0d1b2a] shadow-lg shadow-cyan-950/20" : "text-slate-300 hover:bg-white/10 hover:text-white")}><Icon className="h-[18px] w-[18px]" /><span>{item.label}</span>{selected && <ChevronLeft className="mr-auto h-4 w-4" />}</button>; })}</nav></div>
      ...
      <main className="lg:mr-[286px]">
        ...
        <div className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-8">
          ...
          {isShowcaseSession && active === "pos" && <><ShowcaseInvoiceScenarios /><ShowcasePosInteractions /></>}
          ...
          {isShowcaseSession && showcaseTourOpen && <Card className="border-cyan-200 bg-gradient-to-l from-cyan-50 to-white shadow-md" role="dialog" aria-modal="false" aria-labelledby="showcase-tour-title"><CardContent className="p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="max-w-2xl"><p className="text-xs font-semibold text-cyan-700">خطوة {showcaseTourStep + 1} من {showcaseTourSteps.length}</p><h2 id="showcase-tour-title" className="mt-1 text-xl font-bold text-slate-900">{showcaseTourSteps[showcaseTourStep].title}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{showcaseTourSteps[showcaseTourStep].body}</p></div><div className="flex shrink-0 flex-wrap gap-2"><Button variant="ghost" size="sm" onClick={finishShowcaseTour}>تخطي</Button>{showcaseTourStep > 0 && <Button variant="outline" size="sm" onClick={retreatShowcaseTour}>السابق</Button>}<Button size="sm" onClick={advanceShowcaseTour}>{showcaseTourStep + 1 === showcaseTourSteps.length ? "إنهاء الجولة" : "التالي"}<ChevronLeft className="mr-1 h-4 w-4" /></Button></div></div><div className="mt-4 flex gap-1.5" aria-hidden="true">{showcaseTourSteps.map((_, index) => <span key={index} className={cn("h-1.5 flex-1 rounded-full", index <= showcaseTourStep ? "bg-cyan-500" : "bg-slate-200")} />)}</div></CardContent></Card>}
...
function ModulePanel({ active, organizationId, branchId, jurisdictionId, isShowcase }: { active: string; organizationId: number | null; branchId: number | null; jurisdictionId: number | null; isShowcase: boolean }) {
  ...
  if (active === "operationsHub") return <OperationsHubWorkspace organizationId={organizationId} jurisdictionId={jurisdictionId} />;
...
function ShowcasePosInteractions() {
  const [barcodeScanned, setBarcodeScanned] = useState(false);
  const [returnPreviewOpen, setReturnPreviewOpen] = useState(false);
  const [paymentMethod, setPaymentMethod] = useState<"cash" | "card" | null>(null);
  const [receiptPreviewOpen, setReceiptPreviewOpen] = useState(false);
  const printShowcaseReceipt = () => {
    const printWindow = window.open("", "showcase-receipt", "width=420,height=640");
    if (!printWindow) return;
    printWindow.document.write(`<!doctype html><html dir="rtl" lang="ar"><head><title>إيصال عرض تدريبي</title><style>body{font-family:Arial,sans-serif;padding:24px;color:#172033}h1{font-size:20px;margin:0 0 8px}.demo{color:#0e7490;font-weight:bold}.line{border-top:1px dashed #94a3b8;margin:14px 0}p{margin:6px 0;font-size:14px}</style></head><body><h1>ALDORA | إيصال عرض تدريبي</h1><p class="demo">هذا إيصال محلي للتوضيح فقط — غير صالح مالياً أو ضريبياً.</p><div class="line"></div><p>المرجع: DEMO-INV-2026-1042</p><p>فيتامين C 1000mg × 2 — 168.66 ج.م</p><p>ضريبة القيمة المضافة 14% — 23.62 ج.م</p><div class="line"></div><p><strong>الإجمالي: 192.28 ج.م</strong></p><p>طريقة الدفع: ${paymentMethod === "card" ? "بطاقة (محاكاة)" : "نقداً (محاكاة)"}</p><p>لا توجد حركة بيع أو مخزون أو دفعة مرتبطة بهذا الإيصال.</p><script>window.onload=()=>window.print()</script></body></html>`);
    printWindow.document.close();
  };
  return <div className="grid gap-4 lg:grid-cols-2"><Card className="border-cyan-200 bg-white"><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg"><PackageSearch className="h-5 w-5 text-cyan-700" />مسح باركود تجريبي</CardTitle><p className="text-sm leading-6 text-slate-600">جرّب قراءة رمز محلي محدد مسبقاً. لا تُفتح الكاميرا ولا يُستخدم أي جهاز، ولا تُضاف حركة إلى الفاتورة أو المخزون.</p></CardHeader><CardContent className="space-y-3"><div className="flex flex-col gap-2 rounded-xl border border-dashed border-cyan-200 bg-cyan-50/50 p-3 sm:flex-row sm:items-center sm:justify-between"><code className="text-xs font-semibold text-cyan-900">DEMO-622300-VC1000</code><Button type="button" size="sm" onClick={() => setBarcodeScanned(true)}>محاكاة مسح الباركود</Button></div>{barcodeScanned ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm"><div className="flex items-center gap-2 font-bold text-emerald-900"><CheckCircle2 className="h-4 w-4" />تمت قراءة الرمز في وضع العرض</div><p className="mt-2 text-emerald-800">فيتامين C 1000mg · تشغيلة VC-25C04 · صلاحية 2026-09 · اقتراح FEFO فقط.</p><p className="mt-1 text-xs text-emerald-700">لم يُنشأ سطر فاتورة، ولم يُخصم أي مخزون.</p></div> : <p className="text-xs text-slate-500">اضغط الزر لرؤية تحقق الصنف والتشغيلة بشكل تفاعلي.</p>}</CardContent></Card><Card className="border-violet-200 bg-white"><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg"><WalletCards className="h-5 w-5 text-violet-700" />تسوية دفع تجريبية</CardTitle><p className="text-sm leading-6 text-slate-600">اختر وسيلة الدفع لعرض النتيجة التدريبية. هذا لا يتصل ببنك أو بوابة دفع ولا ينشئ بيعاً أو قيداً مالياً.</p></CardHeader><CardContent className="space-y-3"><div className="rounded-xl border border-violet-100 bg-violet-50/70 p-3 text-sm"><div className="flex items-center justify-between gap-3"><span className="text-slate-600">إجمالي الفاتورة النموذجية</span><span className="font-bold text-violet-950">192.28 ج.م</span></div><p className="mt-1 text-xs text-violet-800">VAT 14% مشمولة في عرض الفاتورة فقط.</p></div><div className="grid grid-cols-2 gap-2"><Button type="button" variant={paymentMethod === "cash" ? "default" : "outline"} onClick={() => setPaymentMethod("cash")}>نقداً</Button><Button type="button" variant={paymentMethod === "card" ? "default" : "outline"} onClick={() => setPaymentMethod("card")}>بطاقة</Button></div>{paymentMethod === "cash" && <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm"><p className="font-bold text-emerald-900">محاكاة نقدية: المبلغ المستلم 200.00 ج.م</p><p className="mt-1 text-emerald-800">الباقي المعروض للعميل: 7.72 ج.م.</p><p className="mt-1 text-xs text-emerald-700">لم يدخل أي مبلغ إلى صندوق نقدي ولم يُسجل إيصال.</p></div>}{paymentMethod === "card" && <div className="rounded-xl border border-sky-200 bg-sky-50 p-3 text-sm"><p className="font-bold text-sky-900">محاكاة بطاقة: تمت الموافقة تجريبياً</p><p className="mt-1 text-sky-800">مرجع عرض فقط: DEMO-CARD-4821 · القيمة 192.28 ج.م.</p><p className="mt-1 text-xs text-sky-700">لا يُرسل طلب إلى بنك أو بوابة دفع، ولا تحفظ بيانات بطاقة.</p></div>}{!paymentMethod && <p className="text-xs text-slate-500">اختر نقداً أو بطاقة لرؤية فرق التسوية في وضع العرض.</p>}</CardContent></Card><Card className="border-sky-200 bg-white"><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg"><Receipt className="h-5 w-5 text-sky-700" />إيصال عرض تدريبي</CardTitle><p className="text-sm leading-6 text-slate-600">معاينة شكل الإيصال النهائي ثم فتح نافذة طباعة محلية فقط. لا يُحفظ إيصال ولا يُرسل إلى طابعة أو خدمة خارجية.</p></CardHeader><CardContent className="space-y-3"><Button type="button" size="sm" variant="outline" onClick={() => setReceiptPreviewOpen(value => !value)}>{receiptPreviewOpen ? "إخفاء معاينة الإيصال" : "معاينة الإيصال التجريبي"}</Button>{receiptPreviewOpen && <div className="rounded-xl border border-sky-200 bg-sky-50 p-3 text-sm text-sky-950"><p className="font-bold">ALDORA | إيصال عرض تدريبي</p><p className="mt-1">DEMO-INV-2026-1042 · الإجمالي 192.28 ج.م</p><p className="mt-1">طريقة الدفع: {paymentMethod === "card" ? "بطاقة (محاكاة)" : "نقداً (محاكاة)"}</p><p className="mt-1 text-xs text-sky-800">غير صالح مالياً أو ضريبياً. لا توجد عملية بيع أو دفعة أو حركة مخزون.</p><Button type="button" className="mt-3" size="sm" onClick={printShowcaseReceipt}>طباعة الإيصال التجريبي محلياً</Button></div>}</CardContent></Card><Card className="border-amber-200 bg-white"><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-lg"><ArrowLeftRight className="h-5 w-5 text-amber-700" />محاكاة مرتجع مبيعات</CardTitle><p className="text-sm leading-6 text-slate-600">مسار توضيحي للبحث عن الفاتورة، سبب الإرجاع، ومراجعة إشعار الخصم وفق سياسة المستهلك والضريبة، دون إصدار مستند فعلي.</p></CardHeader><CardContent className="space-y-3"><div className="rounded-xl border border-amber-100 bg-amber-50/70 p-3 text-sm text-amber-950"><p className="font-bold">الفاتورة المرجعية: DEMO-INV-2026-1042</p><p className="mt-1 text-xs">فيتامين C 1000mg · علبة واحدة · سبب تجريبي: عبوة غير مطابقة لطلب العميل.</p></div><Button type="button" size="sm" variant="outline" onClick={() => setReturnPreviewOpen(value => !value)}>{returnPreviewOpen ? "إخفاء معاينة إشعار المرتجع" : "معاينة إشعار المرتجع"}</Button>{returnPreviewOpen && <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm"><p className="font-bold text-amber-950">إشعار مرتجع تجريبي · DEMO-CN-2026-1042-R1</p><p className="mt-2 text-amber-900">القيمة المعروضة: 96.14 ج.م · VAT مسترد: 11.80 ج.م · الحالة: معاينة فقط.</p><p className="mt-1 text-xs text-amber-800">لا يصدر إشعار، ولا يعاد صنف إلى المخزون، ولا يتغير رصيد عميل أو مؤسسة.</p></div>}</CardContent></Card></div>;
}
