    ...
    <div dir={localization.direction} data-country={localization.countryCode} className="aldo-app-shell min-h-screen text-slate-900">
      <aside className={cn("fixed inset-y-0 right-0 z-40 flex w-[286px] flex-col border-l border-slate-200 bg-[#0d1b2a] text-white transition-transform duration-200 lg:translate-x-0", mobileOpen ? "translate-x-0" : "translate-x-full")}>
        <div className="flex h-20 items-center justify-between border-b border-white/10 px-6">
          <div className="flex items-center gap-3"><div className="aldo-brand-mark grid h-10 w-10 place-items-center rounded-2xl" aria-label="ALDO"><svg viewBox="0 0 48 48" role="img" aria-hidden="true" className="h-7 w-7"><path d="M24 4 40 10v12c0 10.5-6.7 18.2-16 22-9.3-3.8-16-11.5-16-22V10L24 4Z" fill="#0d1b2a" opacity=".92"/><path d="M24 12c-5.5 3.8-8.5 8.1-8.5 12.9 0 5.6 3.8 9.5 8.5 11.1 4.7-1.6 8.5-5.5 8.5-11.1C32.5 20.1 29.5 15.8 24 12Z" fill="#9ff2e4"/><path d="M24 18c-2.4 2.5-3.6 4.8-3.6 7.1 0 2.4 1.4 4.2 3.6 5.2 2.2-1 3.6-2.8 3.6-5.2 0-2.3-1.2-4.6-3.6-7.1Z" fill="#19c5d1"/></svg></div><div><p className="font-bold tracking-tight">ALDO Health Care Eco System</p><p className="text-[11px] text-cyan-200/70">منصة الصيدليات العربية متعددة الدول</p></div></div>
      ...
      <main className="lg:mr-[286px]">
        <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/75 px-4 py-4 backdrop-blur-xl sm:px-8"><div className="flex items-center gap-3"><Button variant="outline" size="icon" className="lg:hidden" onClick={() => setMobileOpen(true)}><Menu className="h-5 w-5" /></Button><div className="min-w-0 flex-1"><p className="text-xs font-medium text-slate-500">الثلاثاء، ١٤ أغسطس ٢٠٢٦</p><h1 className="truncate text-xl font-bold tracking-tight sm:text-2xl">{activeModule.label}</h1></div><div className="hidden w-64 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 sm:flex"><Search className="h-4 w-4 text-slate-400" /><Input value={query} onChange={e => setQuery(e.target.value)} placeholder="ابحث في الوحدات" className="border-0 bg-transparent px-0 shadow-none focus-visible:ring-0" /></div>{localization.branches.length > 0 && <select value={localization.branchId ?? ""} onChange={event => localization.setBranchId(Number(event.target.value))} aria-label="اختيار الفرع" className="hidden h-9 max-w-40 rounded-lg border border-slate-200 bg-white px-2 text-xs sm:block"><option value="" disabled>اختر الفرع</option>{localization.branches.map(branch => <option key={branch.id} value={branch.id}>{branch.nameAr} · {branch.countryCode}</option>)}</select>}<Badge variant="outline" className="hidden bg-white px-3 py-2 text-xs sm:flex">{localization.countryCode} · {localization.currencyCode}</Badge>{isDemo && <Badge variant="outline" className="border-violet-200 bg-violet-50 text-violet-700">عرض فقط</Badge>}<Badge variant="outline" className={cn("hidden px-3 py-2 text-xs sm:flex", online ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700")}>{online ? "متصل" : "مسودات محلية فقط"}</Badge><div className="relative"><Button variant="outline" size="icon" className="relative bg-white" aria-label="الإشعارات" onClick={() => setNotificationOpen(value => !value)}><Bell className="h-4 w-4" />{Boolean(notificationsQuery.data?.unreadCount) && <span className="absolute -left-1 -top-1 grid min-h-4 min-w-4 place-items-center rounded-full bg-rose-500 px-1 text-[9px] font-bold text-white">{notificationsQuery.data?.unreadCount}</span>}</Button>{notificationOpen && <div className="absolute left-0 top-11 z-50 w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-slate-200 bg-white p-3 text-right shadow-xl"><div className="mb-2 flex items-center justify-between"><p className="text-sm font-bold text-slate-900">الإشعارات</p><button className="text-xs text-slate-400 hover:text-slate-700" onClick={() => setNotificationOpen(false)}>إغلاق</button></div>{!user || isDemo ? <p className="rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-500">الإشعارات التشغيلية تظهر بعد تسجيل الدخول. وضع العرض التجريبي لا يقرأ بيانات تشغيلية.</p> : notificationsQuery.isLoading ? <p className="p-3 text-xs text-slate-500">جارٍ تحميل الإشعارات…</p> : notificationsQuery.data?.items.length ? <div className="max-h-80 space-y-2 overflow-auto">{notificationsQuery.data.items.map(item => <button key={item.id} className={cn("w-full rounded-xl border p-3 text-right transition hover:bg-slate-50", item.isRead ? "border-slate-100 bg-white" : "border-cyan-100 bg-cyan-50/50")} onClick={() => { if (!item.isRead) markNotificationRead.mutate({ notificationId: item.id }); }}><div className="flex items-start justify-between gap-3"><span className="text-sm font-semibold text-slate-900">{item.title}</span><span className="text-[10px] text-slate-400">{item.severity}</span></div><p className="mt-1 text-xs leading-5 text-slate-600">{item.body}</p></button>)}</div> : <p className="rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-500">لا توجد إشعارات جديدة.</p>}</div>}</div>{user ? <Badge variant="secondary" className="hidden gap-2 px-3 py-2 sm:flex"><span className="h-2 w-2 rounded-full bg-emerald-500" />{user.name || "مستخدم"}</Badge> : <Button onClick={() => startLogin()} className="hidden bg-[#0d1b2a] sm:flex">تسجيل الدخول</Button>}</div></header>
        <div className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-8">
          {(!user || isDemo) && <div className="flex flex-col gap-4 rounded-2xl border border-cyan-200 bg-cyan-50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-cyan-700" /><div><p className="font-semibold text-cyan-950">وضع العرض الآمن</p><p className="text-sm leading-6 text-cyan-800">استعرض واجهة ALDO بأمان. هذا الوضع للقراءة فقط ولا يصل إلى بيانات تشغيلية أو يرسل تغييرات.</p></div></div><div className="flex shrink-0 items-center gap-2">{!user && <Button onClick={() => startLogin()} className="bg-cyan-700 hover:bg-cyan-800">دخول</Button>}{!user && <Button variant="outline" onClick={() => void startDemo()} className="border-cyan-300 bg-white text-cyan-900 hover:bg-cyan-100">ابدأ العرض التجريبي</Button>}{salesContactUrl ? <a href={salesContactUrl} target="_blank" rel="noopener noreferrer" className="inline-flex h-9 items-center rounded-md border border-cyan-300 bg-white px-3 text-sm font-medium text-cyan-900 transition hover:bg-cyan-100">تواصل لشراء النظام</a> : <span className="text-xs text-cyan-700">قناة الشراء ستُحدد لاحقاً</span>}</div></div>}
...
function CatalogWorkspace() {
  ...
  const jurisdictionId = registry.data?.find(country => country.status === "configured")?.profile?.id
    ?? branchJurisdictions.data?.find(item => item.assignment?.jurisdictionId && item.profile?.active === 1)?.assignment?.jurisdictionId;
