...
function RegionalComplianceWorkspace() {
  ...
  const pending = registry.data?.filter(country => country.status !== "configured").length ?? 22;
  return <WorkspaceShell title="إدارة الدول وحزم الامتثال"><div className="space-y-4"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><strong>بوابة الاعتماد:</strong> لا يُعتبر أي بلد جاهزاً للعمليات المنظمة بمجرد إدخاله. يجب وجود ملف نشط، حزمة معتمدة، أدلة رسمية، وتاريخ مراجعة غير منتهٍ.</div><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-emerald-50 p-4"><p className="text-xs text-emerald-700">دول مهيأة</p><p className="mt-1 text-2xl font-bold text-emerald-900">{configured}</p></div><div className="rounded-xl bg-slate-100 p-4"><p className="text-xs text-slate-600">تحتاج إعداداً أو اعتماداً</p><p className="mt-1 text-2xl font-bold text-slate-900">{pending}</p></div><div className="rounded-xl bg-cyan-50 p-4"><p className="text-xs text-cyan-700">إجمالي السجل العربي</p><p className="mt-1 text-2xl font-bold text-cyan-900">22</p></div></div>{registry.isLoading ? <p className="text-sm text-slate-500">جارٍ تحميل سجل الدول…</p> : registry.error ? <p className="rounded-xl bg-rose-50 p-4 text-sm text-rose-700">تعذر تحميل حالة الدول؛ ستظل العمليات المنظمة محجوبة.</p> : <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{registry.data?.map(country => <div key={country.countryCode} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3"><div><p className="font-medium">{country.countryNameAr}</p><p className="text-xs text-slate-500">{country.countryCode}</p></div><Badge variant={country.status === "configured" ? "default" : "outline"}>{country.status === "configured" ? "مهيأ" : "غير معتمد"}</Badge></div>)}</div>}</div></WorkspaceShell>;
}
