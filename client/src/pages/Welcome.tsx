...
function BrandMark() {
  return (
    <div className="aldo-brand-mark grid h-14 w-14 place-items-center rounded-[1.35rem] shadow-lg shadow-cyan-900/10" aria-label="ALDO">
...
export default function Welcome() {
  ...
  const copy = english
    ? {
        eyebrow: "ALDO HEALTH CARE ECO SYSTEM",
    ...
    : {
        ...
        features: ["عمليات موحّدة", "جاهزية إقليمية", "تجارب عمل إنسانية"],
    ...
    <main dir={direction} className="relative min-h-screen overflow-hidden bg-[#f4f7fb] text-[#0d1b2a]">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-5 py-5 sm:px-8 lg:px-12">
        <header className="flex items-center justify-between gap-4">
          <a href="/" className="flex items-center gap-3" aria-label="ALDO Health Care Eco System">
            <BrandMark />
            <div>
              <p className="text-sm font-bold tracking-tight sm:text-base">ALDO Health Care Eco System</p>
              <p className="text-[11px] text-slate-500">منظومة الرعاية الصحية الشاملة</p>
            </div>
          </a>
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="hidden border-emerald-200 bg-emerald-50 text-emerald-700 sm:flex"><ShieldCheck className="ml-1 h-3.5 w-3.5" /> {english ? "Protected entry" : "دخول محمي"}</Badge>
        ...
        <section className="grid flex-1 items-center gap-12 py-14 lg:grid-cols-[1.08fr_.92fr] lg:gap-20 lg:py-20">
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/80 p-5 shadow-[0_24px_80px_rgba(13,27,42,0.14)] backdrop-blur-xl sm:p-7">
              <div className="flex items-center justify-between border-b border-slate-100 pb-5"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-700">ALDO / WORKSPACE</p><p className="mt-1 text-lg font-bold">{english ? "A clear view of the operation" : "رؤية واضحة للعملية"}</p></div><div className="grid h-10 w-10 place-items-center rounded-xl bg-cyan-50 text-cyan-700"><CheckCircle2 className="h-5 w-5" /></div></div>
              <div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-[#0d1b2a] p-4 text-white sm:col-span-2"><p className="text-xs text-cyan-200">{english ? "One governed workspace" : "مساحة عمل محكومة"}</p><p className="mt-2 text-2xl font-bold">{english ? "Care, operations, control" : "رعاية · تشغيل · رقابة"}</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full w-4/5 rounded-full bg-gradient-to-l from-cyan-300 to-teal-200" /></div></div><div className="rounded-2xl border border-slate-100 bg-slate-50 p-4"><p className="text-xs text-slate-500">{english ? "Access" : "الوصول"}</p><p className="mt-2 font-bold text-emerald-700">{english ? "Scoped & audited" : "محدد ومدقق"}</p></div><div className="rounded-2xl border border-slate-100 bg-slate-50 p-4"><p className="text-xs text-slate-500">{english ? "Demo" : "العرض"}</p><p className="mt-2 font-bold text-violet-700">{english ? "Safe & local" : "آمن ومحلي"}</p></div></div>
